...
describe('Assert Schedule Repository Mock is correct at all for User methods', () => {
    ...
    it('Should get user wrongly: userId does not exists', async () => {
        const repo = new ScheduleRepositoryMock()
        await expect(
            repo.getUser(repo.getUsersLength() + 1),
    ...
    it('Should not create user: user already exists with same ID', async () => {
        ...
        const repo = new ScheduleRepositoryMock()
        await expect(repo.createUser(user)).rejects.toThrowError(
    ...
    it('should getUsersWithAvailabilitiesAndSuitabilities correctly', async () => {
        ...
        const usersWithAvailabilitiesAndSuitabilities =
            await repo.getUsersWithAvailabilitiesAndSuitabilities(usersIds)
        expect(
            Object.keys(usersWithAvailabilitiesAndSuitabilities).length,
        ).toEqual(usersIds.length)
        expect(usersWithAvailabilitiesAndSuitabilities[2].user.id).toEqual(2)
        expect(usersWithAvailabilitiesAndSuitabilities[2].user.role).toEqual(
            ROLE.ADMIN,
        )
        expect(
            usersWithAvailabilitiesAndSuitabilities[2].availabilities.length,
        ).toEqual(0)
        expect(
            usersWithAvailabilitiesAndSuitabilities[2].suitabilities.length,
        ).toEqual(0)

        expect(usersWithAvailabilitiesAndSuitabilities[3].user.id).toEqual(3)
        expect(usersWithAvailabilitiesAndSuitabilities[3].user.role).toEqual(
            ROLE.PROFESSOR,
        )
        expect(
            usersWithAvailabilitiesAndSuitabilities[3].availabilities.length,
        ).toEqual(10)
        expect(
            usersWithAvailabilitiesAndSuitabilities[3].availabilities[9]
                .scheduleFullfilled,
        ).toEqual('2S-3CM-D5@2024(SCS)')
        expect(
            usersWithAvailabilitiesAndSuitabilities[3].suitabilities.length,
        ).toEqual(2)

        expect(usersWithAvailabilitiesAndSuitabilities[4].user.id).toEqual(4)
        expect(usersWithAvailabilitiesAndSuitabilities[4].user.role).toEqual(
            ROLE.ADMIN,
        )
        expect(
            usersWithAvailabilitiesAndSuitabilities[4].availabilities.length,
        ).toEqual(8)
        expect(
            usersWithAvailabilitiesAndSuitabilities[4].suitabilities.length,
        ).toEqual(1)
    })
    ...
    it('should getUsersWithAvailabilitiesAndSuitabilities wrongly: userId does not exist', async () => {
        ...
        await expect(
            repo.getUsersWithAvailabilitiesAndSuitabilities(usersIds),
        ).rejects.toThrowError('No items found for userId')
    })
...
describe('Assert Schedule Repository Mock is correct at all for Possibility methods', () => {
    ...
    it('Should getPossibilitiesByIds wrongly: no items found for certain possibilityId', async () => {
        ...
        await expect(
            repo.getPossibilitiesByIds(possibilityIds),
        ).rejects.toThrowError('No items found for possibilityId')
    })
...
describe('Assert Schedule Repository Mock is correct at all for Availability methods', () => {
    ...
    it('Should not delete availability: availabilityId not found', async () => {
        ...
        await expect(
            repo.deleteAvailability(availabilityId),
        ).rejects.toThrowError('No items found for availabilityId')
    })
    ...
    it('Should not create availability: duplicated availabilityId', async () => {
        ...
        const lastLength = repo.getAvailabilitiesLength()
        await expect(
            repo.createAvailability(availability),
        ).rejects.toThrowError('availabilityId already exists')
        const newLength = repo.getAvailabilitiesLength()
...
describe('Assert Schedule Repository Mock is correct at all for AvFullfilled methods', () => {
    ...
    */
    ...
    it('Should not create AvFullfilled: availability does not exists', async () => {
        ...
        const lastLength = repo.getAvsFullfilledLength()
        await expect(
            repo.createAvFullfilled(avFullfilled),
        ).rejects.toThrowError('No items found for availabilityId')
        const newLength = repo.getAvsFullfilledLength()
    ...
    it('Should not create AvFullfilled: class does not exists', async () => {
        ...
        await expect(
            repo.createAvFullfilled(avFullfilled),
        ).rejects.toThrowError('No items found for classId')
        const newLength = repo.getAvsFullfilledLength()
        expect(newLength).toEqual(lastLength)
        const availabilityAfter = await repo.getAvailability(
            '0a8c5357-1f07-5b24-9845-9318c4000008',
    ...
    */
        ...
        const repo = new ScheduleRepositoryMock()
        const lastLength = repo.getAvsFullfilledLength()
        const availability1Before = await repo.getAvailability(
            avFullfilled1.availabilityId,
        )
        expect(availability1Before.isTaken).toEqual(false)
        const availability2Before = await repo.getAvailability(
            avFullfilled2.availabilityId,
        )
        ...
        expect(availabilityAfter.isTaken).toEqual(true)
        const availabilityAfter2 = await repo.getAvailability(
            avFullfilled2.availabilityId,
        )
