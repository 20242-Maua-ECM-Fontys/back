...
describe('Assert Login User usecase is correct', () => {
  ...
  it('Should throw an error if email is incorrect', async () => {
    ...
    await expect(
      usecase.execute('wrongemail@gmail.com', 'Password1@'),
  ...
  it('Should throw an error if password is incorrect', async () => {
    ...
    await expect(
      usecase.execute('user1@gmail.com', 'WrongPassword'),
    ).rejects.toThrow('Field password is not valid')
  })
  ...
  it('Should throw an error if email or password are missing', async () => {
    ...
    await expect(usecase.execute('user1@gmail.com', '')).rejects.toThrow(
      'Field password is not valid',
    )
  })
  it('Should throw an error if email is in an invalid format', async () => {
    ...
    await expect(
      usecase.execute('invalid-email', 'Password1@'),
    ).rejects.toThrow('Field email is not valid')
  })
  ...
  it('Should throw an error if password is in an invalid format (missing special character)', async () => {
    ...
    await expect(
      usecase.execute('user1@gmail.com', 'Password1'),
    ).rejects.toThrow('Field password is not valid')
  })
