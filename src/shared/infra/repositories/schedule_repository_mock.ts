...
export class ScheduleRepositoryMock implements IScheduleRepository {
    ...
    */
        ...
        for (const user of users) {
            ...
            const availabilitiesWithFullfilled = await Promise.all(
                availabilities.map(async (availability) => {
                    ...
                    const specificClass = this.classes.find(
                        (c) => c.id === classId,
                    )
    ...
    async getPossibilitiesByIds(
        ...
        const possibilities = this.possibilities.filter((p) =>
            possibilitiesIds.includes(p.id),
        )
        ...
        // format the data
        return possibilities.reduce<Record<string, Possibility>>(
            (acc, curr) => {
                acc[curr.id] = curr
                return acc
            },
    ...
    async createAvFullfilled(
        ...
        const possibility = await this.getPossibility(
            avFullfilled.possibilityId,
        ) // possibility exists?
    ...
    async deleteAvsFullfilledByScheduleId(
        ...
        const avsFullfilled = this.avsFullfilled.filter((avFullfilled) => {
            const possibility = this.possibilities.find(
                (p) => p.id === avFullfilled.possibilityId,
            )
            return possibility?.scheduleId !== scheduleId
        })
        const avsFullfilledIds = avsFullfilled.map(
            (avFullfilled) => avFullfilled.availabilityId,
        )
